...
class QuestSinglePiperNode(Node):
    ...
    def tick(self):
        ...
        if current is not None and should_reset_vr_reference(
            self.previous_b_gate_active, b_gate_active
        ):
            # Use the already limited pose. The raw APK matrix can jump when
            # head tracking recenters; accepting it here would inject a
            # discontinuity exactly when B re-enables control.
        ...
        if vr_active and current is not None:
            ...
            target, clipped = limit_target_translation_with_margin(
                target, 1.0,
