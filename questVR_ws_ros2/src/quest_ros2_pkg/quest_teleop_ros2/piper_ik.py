...
class PiperIK:
    ...
    def __init__(self, urdf_path: str, max_joint_jump_deg: float = 120.0):
        if not os.path.isfile(urdf_path):
            raise FileNotFoundError(urdf_path)
        if max_joint_jump_deg <= 0 or max_joint_jump_deg > 180:
            raise ValueError("max_joint_jump_deg must be in (0, 180]")
        self.max_joint_jump_rad = np.deg2rad(float(max_joint_jump_deg))
    ...
    def solve_with_status(self, target: np.ndarray):
        ...
        if q.size != 6 or not np.all(np.isfinite(q)):
            return None, "solver_failed"
        if np.max(np.abs(q - self.last_q)) > self.max_joint_jump_rad:
